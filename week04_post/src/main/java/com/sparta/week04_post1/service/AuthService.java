...
import com.sparta.week04_post1.repository.RefreshTokenRepository;
...
import com.sparta.week04_post1.jwt.TokenProvider;
...
//** 코드상으로는 전혀 구현된게 없음 --> CustomUserDetailsService : 내부적으로 수행되는 검증 과정
//** 인증이 완료된 authentication : Member ID 가 들어있음
//** 인증 객체를 바탕으로 Access Token + Refresh Token 을 생성
//** Refresh Token 은 저장하고, 생성된 토큰 정보를 클라이언트에게 전달
    @Transactional
    public TokenDto login(MemberRequestDto memberRequestDto) {

// 1. Login ID/PW 를 기반으로 AuthenticationToken 생성 : return new UsernamePasswordAuthenticationToken(email, password)
        UsernamePasswordAuthenticationToken authenticationToken = memberRequestDto.toAuthentication();

// 2. 실제로 검증 (사용자 비밀번호 체크) 이 이루어지는 부분
//    authenticate 메서드가 실행이 될 때, CustomUserDetailsService 에서 만들었던 loadUserByUsername 메서드가 실행됨
        //Authentication
        //** UsernamePasswordAuthenticationToken (인증 정보 객체) : 사용자가 입력한 Login ID, PW 로 생성
        //** 아직 인증이 완료된 객체가 아님
                                    //**AuthenticationManager 에서 authenticate 메소드의 파라미터로 넘겨서 검증 후에, Authentication 를 받습니다.
...
// 4. RefreshToken 저장
                                            //** Builder :  UserDetails 의 유저 정보가 서로 일치하는지 검사
